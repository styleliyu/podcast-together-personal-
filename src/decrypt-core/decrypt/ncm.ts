import {
  ...
  WriteMetaToFlac,
  WriteMetaToMp3,
} from '@decrypt-core/decrypt/utils';
...
import { DecryptResult } from '@decrypt-core/decrypt/entity';
...
async function compressCoverImage(buffer: ArrayBuffer, mime: string): Promise<ArrayBuffer> {
  if (typeof createImageBitmap !== 'function' || typeof document === 'undefined') {
    return buffer;
  }

  const source = new Blob([buffer], { type: mime || 'image/jpeg' });
  const image = await createImageBitmap(source);
  try {
    const targetRatio = Math.min(0.75, Math.sqrt((1 << 23) / buffer.byteLength));
    ...
    return compressed ? compressed.arrayBuffer() : buffer;
  } finally {
    image.close();
  }
}
...
class NcmDecrypt {
    ...
    if (this.oriMeta.albumPic)
      try {
        ...
        while (this.image && this.image.buffer.byteLength >= 1 << 24) {
          const nextBuffer = await compressCoverImage(this.image.buffer, this.image.mime);
          if (nextBuffer.byteLength >= this.image.buffer.byteLength) break;
          this.image.buffer = nextBuffer;
  ...
  async _writeMeta() {
    ...
    if (shouldWrite || this.newMeta.picture) {
      ...
      if (!this.audio) throw Error('invalid sequence');
